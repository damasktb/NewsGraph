...
The graphing library used to draw the maps (d3.js) includes an implementation of \possessivecite{forcedirected} force-directed layout algorithm, and provides several parameters which can be used to alter the forces applied to some or all of the nodes in the graph. The two forces we will adjust to optimise the force-direction for metro maps are as follows:
...
%   \If{$\theta > \frac{\pi}{2}$}{
%   	$\theta \gets \pi - \theta$ \tcc*[f]{We want the complement of this angle}
%   }
   $ls \gets ls + \theta$ \;
   }
}
