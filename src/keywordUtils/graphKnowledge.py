...
class NewsGraphResult:
  """
  Class encapsulating result XML from Google's knowledgegraph API
  """
  NAME  = 0
  TYPES = 1
  DESCR = 2
  ...
  def __init__(self, query, candidates):
    """
    Extract the fields we want - name, description, and score.
    """
    self.query = query
    self.candidates = []
  ...
  def nth(self, n):
    """
    Get the nth result for this query
    """
  ...
  def isPerson(self):
    """
    Make an educated guess as to whether this entity is a person
    """
    if self.candidates:
...
class NewsGraphKnowledge:
  """
  Class for performing entity disambiguation with knowledge graph
  """
  LOG = False
  ...
  def __init__(self):
    """
    Load the API key so we can make requests
    """
  ...
  def query(self, queryText, limit=3):
    """
    Return the results of a query against the API
    """
    params = {
      'query': queryText,
      'limit': limit,
  ...
  def aliasEntities(self, entities, certainty=3):
    """
    Generate the set of unambiguous aliases for entities in an article
    """
    uncertain = []
    mapping = {}
    people = {}
